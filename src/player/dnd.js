const data = {
    heading: [
        "@name is going to roll the stats for a fucking",
        "Check this shit out, @name is going to be a fucking",
        "You think your character is cool? @name's character is a fucking",
    ],
    response: [
...
function getDndCharacter(user) {
    const heading = getRandomElement(data.heading).replace('@name', user);
    const adjective = getRandomElement(data.adjective);
    const race = getRandomElement(data.race);
    const dclass = getRandomElement(data.dclass);
    const location = getRandomElement(data.location);
    const backstory = getRandomElement(data.backstory);

    return `${heading} ${adjective} ${race} ${dclass} from ${location} who ${backstory}`;
}
...
export default getDndCharacter;